...
const TEMPLATE = (banner, imports, defs, exp1, exp2) => `
${banner}
${imports}
...
function compile() {
    ...
    for (var x of extensions) {
        ...
        imports.push(
            `import Main from '../../src/extensions/${x.name}/main.js'`
        )
        imports.push(
            `Main.__name__ = '${x.name}'`
        )
        ...
        let fn = x.name + postfix
        if (!fs.existsSync(dir)) fs.mkdirSync(dir)
        fs.writeFileSync(dir + fn, source)

        sub_idices.push([x.name, '.' + dir + fn])

        console.log(' [OK]'.green)
    }

    build_main_index(sub_idices)
...
function build_main_index(subs) {
    ...
    for (var x of subs) {
        imports.push(`import ${name2Name(x[0])} from '${x[1]}'`)
    }

    for (var x of subs) {
        exports.push(`'${x[0]}': ${name2Name(x[0])}`)
    }
    ...
    if (argv.prod) {
        fs.writeFileSync('./src/index_prod.js', source)
    } else {
        fs.writeFileSync('./src/index_dev.js', source)
    }
}
